...
def setupkern(up: dict[str, Any]) -> dict[str, Any]:
    ...
    up["close"] = cv2.getStructuringElement(
        cv2.MORPH_RECT, (up["close_width"], up["close_height"])
    )
...
def svsetup(up: dict[str, Any], finf: dict[str, Any]) -> dict[str, Any]:
    ...
    if savevideo == "tif":
        ...
        try:
            ...
            logging.error(
                "I cannot save iterated video results due to missing tifffile module \n"
        ...
        svh["video"] = (
            TiffWriter(str(up["odir"] / "video.tif")) if "rawscaled" in pshow else None
        )
        svh["thres"] = (
            TiffWriter(str(up["odir"] / "thres.tif")) if "thres" in pshow else None
        )
        svh["despeck"] = (
            TiffWriter(str(up["odir"] / "despk.tif")) if "thres" in pshow else None
        )
        svh["erode"] = (
            TiffWriter(str(up["odir"] / "erode.tif")) if "morph" in pshow else None
        )
        svh["close"] = (
            TiffWriter(str(up["odir"] / "close.tif")) if "morph" in pshow else None
        )
        # next line makes big file
        svh["detect"] = (
            None  # TiffWriter(join(tdir,'detect.tif')) if showfinal else None
        )
        ...
        if wfps < 3:
            logging.warning(
                "VLC media player had trouble with video slower than about 3 fps"
            )
        ...
        """
        if up.get("wienernhood"):
            svh["wiener"] = cv2.VideoWriter(
                str(up["odir"] / "wiener.avi"), cc4, wfps, (y, x), False
            )
        else:
            svh["wiener"] = None

        svh["video"] = (
            cv2.VideoWriter(str(up["odir"] / "video.avi"), cc4, wfps, (y, x), False)
            if "rawscaled" in pshow
            else None
        )
        svh["thres"] = (
            cv2.VideoWriter(str(up["odir"] / "thres.avi"), cc4, wfps, (y, x), False)
            if "thres" in pshow
            else None
        )
        svh["despeck"] = (
            cv2.VideoWriter(str(up["odir"] / "despk.avi"), cc4, wfps, (y, x), False)
            if "thres" in pshow
            else None
        )
        svh["erode"] = (
            cv2.VideoWriter(str(up["odir"] / "erode.avi"), cc4, wfps, (y, x), False)
            if "morph" in pshow
            else None
        )
        svh["close"] = (
            cv2.VideoWriter(str(up["odir"] / "close.avi"), cc4, wfps, (y, x), False)
            if "morph" in pshow
            else None
        )
        svh["detect"] = (
            cv2.VideoWriter(str(up["odir"] / "detct.avi"), cc4, wfps, (y, x), True)
            if "final" in pshow
            else None
        )
...
def setupof(U: dict[str, Any], finf: dict[str, Any]) -> tuple:
    ...
    # %% GMM
        ...
        if cv2 is None:
            raise ImportError("OpenCV is needed")
        gmm = cv2.createBackgroundSubtractorKNN(
            history=U["gmm_nhistory"], detectShadows=True
        )
    elif U["ofmethod"] == "gmg":
        if cv2 is None:
            raise ImportError("OpenCV is needed")
        ...
        try:
            gmm = cv2.createBackgroundSubtractorGMG(
                initializationFrames=U["gmm_nhistory"]
            )
...
def setupfigs(
    finf: dict[str, Any], fn: Path, U: dict[str, Any]
    ...
    if "threscolor" in U["pshow"] and U.get("ofmethod") in {"hs", "farneback"}:
        ...
        axom = fg.gca()
        hiom = axom.imshow(
            np.zeros((finf["super_y"], finf["super_x"])),
            vmin=1e-5,
            vmax=1,  # arbitrary limits
            origin="upper",  # upper like OpenCV
    ...
    stat[["mean", "median", "variance"]] = np.zeros(
