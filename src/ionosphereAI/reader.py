...
import imageio
# tifffile is excruciatingly slow on each file access
...
def setscale(fn: Path, up: dict[str, Any], finf: dict[str, Any]) -> dict[str, Any]:
    """
    ...
    if mod:
        ...
        assert cdiff > 0, (
            f"raw limits do not make sense  lower: {up['rawlim'][0]}   upper: {up['rawlim'][1]}"
        )

        if cdiff < 20:
            raise ValueError(
                "your video may have very poor contrast and not work for auto detection"
            )
...
def samplepercentile(fn: Path, pct: float, up: dict[str, Any], finf: dict[str, Any]):
    """
    for 16-bit files mainly
    """
    isamp = (finf["nframe"] * np.array([0.1, 0.25, 0.5, 0.75, 0.9])).astype(
        int
    )  # pick a few places in the file to touch
...
def read_spool(
    fn: Path, ifrm: int, twoframe: bool, finf: dict[str, Any], zerocols: int
):
    """
