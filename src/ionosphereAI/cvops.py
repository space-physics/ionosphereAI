...
def dooptflow(
    ...
    if up["ofmethod"] == "hs":
        ...
        u, v = HornSchunck(
            frame[1, :, :], frame[0, :, :], up["hs_smooth"], up["hs_iter"]
        )
    ...
    stat["median"].iat[jfrm] = np.median(
        ofmag
    )  # we don't know if it will be index or ut1 in index
...
def dothres(
    ...
    if not isgmm:  # OptFlow based
        if up["thresmode"] == "median":
            ...
            lowthres = (
                P.getfloat("blob", "ofthresmin") * medianflow
            )  # median is scalar!
...
def domorph(mot, svh: dict[str, Any], up: dict[str, Any]):
    """
