...
from .dio import get_sensor_config, get_file_info, keyhandler, savestat
...
def procaurora(
    file: Path, P: ConfigParser, U: dict[str, Any], finf: dict[str, Any]
) -> pandas.DataFrame:
    ...
    if finf["reader"] == "spool":
        ...
        flist = finf["flist"][finf["frameind"]].astype(str)  # type: ignore
    ...
    if finf["reader"] == "spool":
        ...
        zy, zx = zoom(
            get_frames(finf["path"] / flist[0], ifrm=0, finf=finf, up=U, svh=svh)[
                0, :, :
            ],
            0.1,
            order=0,
        ...
        setupimgh5(
            U["detfn"],
            np.ceil(N.size / U["previewdecim"]) + 1,
            zy,
            zx,
            np.uint16,
            writemode="w",
