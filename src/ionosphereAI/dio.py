...
def read_cv2(fn: Path) -> dict[str, Any]:
    ...
    finf = {"reader": "cv2", "nframe": vidparam["nframe"]}
...
def read_spool(fn: Path, U: dict[str, Any], f0: dict[str, Any]) -> dict[str, Any]:
    if spoolparam is None:
        raise ImportError("pip install dmcutils")

    finf = spoolparam(
        fn.parent / SPOOLINI, U["super_x"], U["super_y"], U.get("cmos_stride", 0)
    )
    finf = {**finf, **f0}
