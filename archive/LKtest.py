...
def main(
    ...
    # get params
    if BigExt == ".DMCdata":
        finf = rdr.getDMCparam(BigFN, xyPix[0], xyPix[1], xyBin[0], xyBin[1], FrameInd)
        fid = fn.open("rb")
        (grayRef, rawIndRef) = rdr.getDMCframe(
            fid, 0, BytesPerFrame, PixelsPerImage, Nmetadata, SuperX, SuperY
        )
        ...
        if all(FrameInd == 0):
            FrameInd = np.arange(
                0, nFrame, 1, dtype=int
            )  # has to be numpy for > comparison
    ...
    for iFrm in FrameInd:
        if BigExt == ".DMCdata":
            (gray, rawIndGray) = rdr.getDMCframe(
                fid, iFrm + 1, BytesPerFrame, PixelsPerImage, Nmetadata, SuperX, SuperY
            )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "videofile", help="video data file to process", type=str, default=None
    )
    ...
    p.add_argument(
        "--frameind",
        help="frame indices to process (start,stop)",
        type=int,
        default=(0, 0),
    )
    p.add_argument(
        "--play",
        help="frame time in seconds for playback (arbitrary)",
        type=float,
        default=0.01,
    )
    p.add_argument(
        "--clim",
        help="arbitrary pixel value limits for playback display (low,high)",
        type=float,
        default=(None, None),
    )
    p.add_argument(
        "--framerate",
        help="frame rate of rate data (at time camera ran)",
        type=float,
        default=None,
    )
    p.add_argument(
        "--tstart",
        help="time camera started that night yyyy-mm-ddTHH:MM:SS.fffZ",
        type=str,
        default=None,
    )
    a = p.parse_args()
