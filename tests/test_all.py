...
def test_parse_params(tmp_path):
    with pytest.raises(FileNotFoundError):
        ia.io.get_sensor_config(tmp_path)

    pfn = R.parent / 'hst0.ini'
    P = ia.io.get_sensor_config(pfn)
...
def test_file_read(tmp_path):
    ...
    with pytest.raises(FileNotFoundError):
        ia.io.get_file_info(tmp_path, {})

    up = {'header_bytes': 4, 'xy_pixel': (512, 512), 'xy_bin': (1, 1),
          'twoframe': True}
    finf = ia.io.get_file_info(fn, up)
